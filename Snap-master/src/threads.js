...
ThreadManager.prototype.stopAll = function (excpt) {
    ...
    Process.prototype.CTLstopSpeaking();
};
...
// Custom CTL-added code
Process.prototype.CTLmove = function(index1, list, index2) {
    this.assertType(list, 'list');
    if (index1 === '' || index2 === '') return '';
    if (this.inputOption(index1) === 'last') index1 = list.length();
    if (this.inputOption(index2) === 'last') index2 = list.length();
    
    var element = Process.prototype.reportListItem(index1, list);
    Process.prototype.doDeleteFromList(index1, list);
    Process.prototype.doInsertInList(element, index2, list);
};

Process.prototype.CTLdelete = function(index1, list) {
    this.assertType(list, 'list');
    if (index1 === '') return '';
    if (this.inputOption(index1) === 'last') index1 = list.length();
...
CTLcallServer = function(string, url, param="") {
    if(string instanceof List) {
        string = string.itemsArray().join(' ');
    }
    string = string.replace('?', '');

    var response = 'Error';
    url = 'http://localhost:8000/?' + url;
    //http://localhost:8000/?Query+What%20is%20autism?
    try {
        const xhttp = new XMLHttpRequest();
        xhttp.open("GET", url + '+' + string + '+' + param, false);
        xhttp.send();
...
Process.prototype.CTLstopSpeaking = function() {
    CTLcallServer("", "stopspeak")
};

//Not used, and probably not working
Process.prototype.CTLButtonHandler = function() {
    ledID = 25
    buttonID = 23
    
    try {
    const Gpio = require('onoff').Gpio; // Gpio class
    const button = new Gpio(buttonID, 'in', 'both');       // Export GPIO17 as an output
    
    button.watch((err, value) => console.log("Button"));

    } catch(err) {
        console.log("Failed GPIO load");
    }
};

Process.prototype.CTLanswerQ = function (string) {
    return CTLcallServer(string, "query");
};

Process.prototype.CTLsearchFor = function(string) {
    return CTLcallServer(string, "ematch");
};

Process.prototype.CTLfindComplete = function(string, type, normalized, numMatches, bigrams) {
    var params = "";
    if(normalized) params+="normalize+";
    else params+="+";

    if(numMatches) params+=(numMatches.toString()+"+");
    else params+="+";

    if(bigrams) params+="bigrams";

    return Process.prototype.CTLfind(string, type, params);
}

Process.prototype.CTLfind = function(string, type, param="") {
    if (type=="Overlap similarity") {
        return CTLcallServer(string, "inmatch overlap", param)
    }

    if (type=="Cosine similarity") {
        return CTLcallServer(string, "inmatch cosine", param)
    }
    return CTLcallServer(string, "ematch");
};

Process.prototype.CTLfindBigram = function(string, type) {
    return Process.prototype.CTLfind(string, type, "bigrams++")
};

Process.prototype.CTLspeak = function(string) {
    return CTLcallServer(string, "speak");
};
